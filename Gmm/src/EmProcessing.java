	...
	private ArrayList<Double> solveNormalRandomHard() {
		...
		for (int it = 0; it < iterations; it++) {
			...
			for (int j = 0; j < points.length; j++) {
				double p0 = GaussianOperations.gaussianPdf(points[j], params[0], params[1]);
				double p1 = GaussianOperations.gaussianPdf(points[j], params[2], params[3]);
